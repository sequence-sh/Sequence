    ...
    public class YamlObjectWrapper : IDocumented
    {
        ...
        public YamlObjectWrapper(Type processType, DocumentationCategory category)
        {
            ...
            RelevantProperties = processType.GetProperties()
                .Select(propertyInfo=> (propertyInfo, YamlMemberAttribute: propertyInfo.GetCustomAttribute<YamlMemberAttribute>()))
                .Where(x=>x.YamlMemberAttribute != null)
                // ReSharper disable once ConstantConditionalAccessQualifier
                .OrderBy(x=>x.YamlMemberAttribute?.Order)
                .ThenBy(x=>x.propertyInfo.Name).Select(x=>x.propertyInfo).ToList();
